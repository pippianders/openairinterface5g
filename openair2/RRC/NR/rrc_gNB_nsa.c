...
void rrc_add_nsa_user(gNB_RRC_INST *rrc,struct rrc_gNB_ue_context_s *ue_context_p, x2ap_ENDC_sgnb_addition_req_t *m) {
  ...
  fill_default_reconfig(carrier->servingcellconfigcommon, scc, reconfig_ies, UE->secondaryCellGroup, UE->UE_Capability_nr, configuration, ue_context_p->local_uid);

  ue_context_p->ue_id_rnti = UE->secondaryCellGroup->spCellConfig->reconfigurationWithSync->newUE_Identity;
  ...
  nr_pdcp_add_drbs(
      ctxt.enb_flag, ctxt.rntiMaybeUEid, UE->rb_config->drb_ToAddModList, (UE->integrity_algorithm << 4) | UE->ciphering_algorithm, kUPenc, kUPint, UE->secondaryCellGroup->rlc_BearerToAddModList);

  nr_rrc_rlc_config_asn1_req(&ctxt,
                             get_softmodem_params()->sa ? UE->rb_config->srb_ToAddModList : NULL,
                             UE->rb_config->drb_ToAddModList,
                             UE->rb_config->drb_ToReleaseList,
                             (LTE_PMCH_InfoList_r9_t *)NULL,
                             UE->secondaryCellGroup->rlc_BearerToAddModList);
