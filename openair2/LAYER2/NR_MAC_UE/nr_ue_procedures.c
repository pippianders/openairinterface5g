    ...
    switch(pucchres->format.present) {
      ...
      case NR_PUCCH_Resource__format_PR_format2 :
        ...
        pucch_pdu->prb_size = compute_pucch_prb_size(2, pucchres->format.choice.format2->nrofPRBs,
                                                     n_uci,pucch_Config->format2->choice.setup->maxCodeRate,
      ...
      case NR_PUCCH_Resource__format_PR_format3 :
        ...
        pucch_pdu->prb_size = compute_pucch_prb_size(3,pucchres->format.choice.format3->nrofPRBs,
                                                     n_uci,pucch_Config->format3->choice.setup->maxCodeRate,
...
NR_PUCCH_Resource_t *find_pucch_resource_from_list(struct NR_PUCCH_Config__resourceToAddModList *resourceToAddModList, long resource_id)
{
  ...
  while (k < num_res -1) {
    ...
    while (l < num_res - 1 - k) {
      ...
      if (curr_start > next_start ||
          (curr_start == next_start && curr_length < next_length)) {
        // swap resources
        ...
        switch (next_resource->format.present) {
          case NR_PUCCH_Resource__format_PR_format0 :
            if(res[i].n_sr > 0 && res[i].n_harq == 0 &&
              ...
              AssertFatal(false, "We cannot multiplex more than 1 SR into a PUCCH F0 and we don't expect more than 1 PUCCH with HARQ per slot\n");
            break;
          case NR_PUCCH_Resource__format_PR_format1 :
            if(res[i].n_sr > 0 && res[i].n_harq == 0 &&
               res[i + 1].n_sr == 0 && res[i + 1].n_harq > 0)
          ...
          case NR_PUCCH_Resource__format_PR_format3 :
            if (res[i + 1].n_csi > 0) {
              ...
              AssertFatal(res[i].n_harq > 0 && res[i + 1].n_harq == 0,
                          "There is CSI in next F2 or above resource, since there is no CSI in current one, we expect HARQ in there and not in next\n");
...
bool get_downlink_ack(NR_UE_MAC_INST_t *mac,
                      frame_t frame,
                      int slot,
                      PUCCH_sched_t *pucch)
{
          ...
          if (current_harq->dai > NR_DL_MAX_DAI) {
            ...
            if (res_ind != -1 && res_ind != current_harq->pucch_resource_indicator)
              LOG_E(MAC, "Value of pucch_resource_indicator %d not matching with what set before %d (Possibly due to a false DCI) \n",
                    current_harq->pucch_resource_indicator, res_ind);
            else{
              ...
              AssertFatal(res_ind == -1 || res_ind == temp_ind, "Current resource index %d does not match with previous resource index %d\n", temp_ind, res_ind);
              res_ind = temp_ind;
              pucch->n_CCE = current_harq->n_CCE;
              pucch->N_CCE = current_harq->N_CCE;
              is_common = current_harq->is_common;
              pucch->delta_pucch = current_harq->delta_pucch; // TODO this doesn't make any sense here
  ...
  */
  ...
  NR_PUCCH_Config_t *pucch_Config = current_UL_BWP->pucch_Config;
  if (is_common || !pucch_Config || !pucch_Config->resourceSetToAddModList || pucch_Config->resourceSetToAddModList->list.array[0] == NULL) {
    ...
    long *acknack_resource_id = pucch_Config->resourceSetToAddModList->list.array[resource_set_id]->resourceList.list.array[res_ind];
                                         ...
                                         int slot)
{
  ...
  for (int SR_resource_id =0; SR_resource_id < pucch_Config->schedulingRequestResourceToAddModList->list.count;SR_resource_id++) {
    ...
    if ((sfn_sf - SR_offset) % SR_period == 0) {
      ...
      NR_PUCCH_Resource_t *sr_pucch = find_pucch_resource_from_list(pucch_Config->resourceToAddModList, *SchedulingRequestResourceConfig->resource);
...
int compute_csi_priority(NR_UE_MAC_INST_t *mac, NR_CSI_ReportConfig_t *csirep)
{
  ...
  int k = csirep->reportQuantity.present == NR_CSI_ReportConfig__reportQuantity_PR_cri_RSRP ||
          csirep->reportQuantity.present == NR_CSI_ReportConfig__reportQuantity_PR_ssb_Index_RSRP ? 0 : 1;
