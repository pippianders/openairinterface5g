...
void nr_ue_pucch_scheduler(module_id_t module_idP, frame_t frameP, int slotP, void *phy_data)
{
  ...
  // SR
  if (trigger_periodic_scheduling_request(mac, &pucch[0], frameP, slotP)) {
    mac->nr_ue_emul_l1.num_srs = 1;
    num_res++;
  ...
  if(csi_res > 0) {
    mac->nr_ue_emul_l1.num_csi_reports = pucch[num_res].n_csi; // it assumes a single CSI resource, ok for now but not in the future
    num_res += csi_res;
  }
  ...
  for (int j = 0; j < num_res; j++) {
    if(pucch[j].n_harq + pucch[j].n_sr + pucch[j].n_csi != 0) {
      LOG_D(NR_MAC,"%d.%d configure pucch, O_ACK %d, O_SR %d, O_CSI %d\n", frameP, slotP, pucch[j].n_harq, pucch[j].n_sr, pucch[j].n_csi);
      ...
      pthread_mutex_unlock(&ul_config->mutex_ul_config);
      nr_ue_configure_pucch(mac, slotP, mac->crnti, //FIXME not sure this is valid for all pucch instances
                            &pucch[j], pucch_pdu);
      nr_scheduled_response_t scheduled_response;
